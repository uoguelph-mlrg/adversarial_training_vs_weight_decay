...
class Linear(Layer):
    ...
    def set_input_shape(self, input_shape):
        ...
        with tf.variable_scope('logit', reuse=tf.AUTO_REUSE):
            ...
            '''
            # x = tf.reshape(x, [tf.shape(x)[0], -1]) # this is accomplished by
            # Flatten layer
...
def _fully_connected(x, out_dim):
    ...
    w = tf.get_variable(
        ...
        initializer=tf.initializers.variance_scaling(distribution='uniform'))
    # initializer=tf.uniform_unit_scaling_initializer(factor=1.0)) # deprecated
    b = tf.get_variable('biases', [out_dim],
                        initializer=tf.initializers.constant())
    # initializer=tf.constant_initializer()) # deprecated
    return tf.nn.xw_plus_b(x, self.w, self.b)  # return tf.matmul(x, w)
