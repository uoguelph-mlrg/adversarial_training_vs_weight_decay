...
if __name__ == '__main__':
    ...
    losses = tf.nn.softmax_cross_entropy_with_logits(
        logits=logits, labels=one_hot_vec)
    ...
    # RELAXGradientEstimator here
    # grad_estimate = tf.reduce_mean(grad_estimates, axis=0)
    grad_estimate = tf.gradients(losses, x)[0]
    ...
    '''
    def get_grad(pt, summary_writer, merge_op, i):
        feed_dict = {x: pt}
        loss, dl_dx_, summ = sess.run([tf.reduce_mean(losses),
                                       tf.reduce_mean(grad_estimate, axis=0), merge_op], feed_dict)
        summary_writer.add_summary(summ, i)
        summary_writer.flush()
        return loss, dl_dx_
    ...
    for i in range(max_iters):
        ...
        while current_lr > MIN_LR:
            ...
            num_queries += 1
            eval_logits_ = sess.run(logits, {x: proposed_adv})[0]
        ...
        log_text = 's %05d: l %.9f e %.3f eps-d %.2E lr %.2E (t %.2f)' \
